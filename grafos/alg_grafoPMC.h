                           ...
                           vector<typename GrafoP<tCoste>::vertice> &P)
{
   ...
   //Caminos directos hasta destino
   P = vector<vertice>(n, destino);
   //Incluimos el destino en el vector de vertices usados
   S[destino] = true;

   //Calcular caminos de coste minimo desde cada vertice
   for (size_t i = 1; i <= n - 2; i++)
   {
      //Seleccionar vertice w no incluido en S,
      //con menor coste hasta el destino
      tCoste costeMin = GrafoP<tCoste>::INFINITO;
      for (v = 0; v <= n - 1; v++)
      {
         if (!S[v] && D[v] <= costeMin)
      ...
      S[w] = true; //Incluir vertice W en los usados S
      //Recalcular el coste desde cada v no incluido en S hasta D a traves de w
      for (v = 0; v <= n - 1; v++)
      {
         if (!S[v])
         {
...
template <typename tCoste>
tCoste pseudoDiametro(const GrafoP<tCoste> &G)
{
   typedef typename GrafoP<tCoste>::vertice vertice;

   int n = G.numVert();

   //Creamos la matriz de caminos entre vertices
   matriz<typename GrafoP<tCoste>::vertice> P(n);
   //Obtenemos la matriz de caminos de coste minimo mediante el algoritmo de Floyd
   matriz<tCoste> costes_minimos = Floyd(G, P);

   tCoste max1, max2;
   tCoste diametro = GrafoP<tCoste>::INFINITO;

   //Recorremos la lista de nodos que pueden ser pseudocentro
   for (vertice centro = 0; centro <= n - 1; centro++)
   {
      max1 = 0;
      max2 = 0;
      //Recorremos la lista de caminos desde el centro hasta los dos extremos
      for (vertice extremo = 0; extremo <= n - 1; extremo++)
      {
         //Si el coste hasta el nodo es mayor que el extremo menor lo cambiamos
         if (G[centro][extremo] > max1)
         {
            //Si el coste es mayor que hasta el extremo mayor lo cambiamos,
            //y modificamos el extremo menor
            if (G[centro][extremo] > max2)
            {
               max1 = max2;
               max2 = G[centro][extremo];
            }
            //Si no es mayor que el extremo mayor, solo modificamos el menor
            else
            {
               max1 = G[centro][extremo];
            }
         }
         //Una vez tenemos los dos extremos, calculamos la suma, y
         //la almacenamos en caso de que sea menor
...
template <typename tCoste>
bool subvencion(GrafoP<tCoste> &G)
{
   typedef typename GrafoP<tCoste>::vertice vertice;
   const size_t n = G.numVert();
   matriz<tCoste> A(n); // matriz de costes mínimos
   ...
   for (vertice k = 0; k < n; k++)
   {
      for (vertice i = 0; i < n; i++)
      {
         ...
         if (k == i)
         {
            if (A[k][i] < GrafoP<tCoste>::INFINITO)
            {
               return false;
            }
         }
