...
float calcular_ruta(const GrafoP<tCoste> &G, int capacidad, vertice almacen)
{
    ...
    vector<vertice> P;
    vector<tCoste> hasta_almacen = DijkstraInv(G, almacen, P);
    vector<tCoste> desde_posicion;
    ...
    while (clientes_por_servir > 0)
    {
        if (cajas_actuales > 0)
        {
            desde_posicion = Dijkstra(G, actual, P);
            cliente_sig = encontrar_siguiente(desde_posicion, actual, clientes_servidos);
            dist_total += desde_posicion[cliente_sig];
        ...
        else
        {
            ...
            dist_total += hasta_almacen[actual];
...
int main()
{
    ...
    std::cout << "Average time: " << average; //1796378 us
    //                                           861227
}
